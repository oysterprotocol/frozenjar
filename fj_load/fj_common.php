...
//VAR
$fj_conn = new mysqli("localhost", "blank", "blank", "fj_load");
...
function fj_return($fj_return_code) {
	//2 - try block again later
	//3 - upload failed, reattempt entire file
	echo $fj_return_code;
	exit;
}
function fj_load_select() {
	...
	$fj_self = 4000+intval(str_ireplace("FJ", "", gethostname()));
	$fj_load_select = array();
	foreach ($fj_load_range as $fj_load) {
		if ($fj_load==$fj_self) $fj_server = false;
		else $fj_server = array($fj_load, "fj_load");
